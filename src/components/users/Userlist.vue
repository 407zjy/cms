        ...
        <el-card class="box-card">
            ...
            <el-table :data="userlist" v-if="flag">
                ...
                <el-table-column
                    ...
                    width="60px"
                ></el-table-column>
                <el-table-column label="充值金币" width="180px">
                    <!-- 作用域插槽 -->
                ...
                >
                    <!-- 时间调用全局定义的过滤器 -->
                    <template slot-scope="scope">
                        <span v-if="scope.row.expiration">{{
                            scope.row.expiration | dateFormat
                        }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="开通相册" width="80px">
                    <!-- 作用域插槽 -->
                    <template slot-scope="scope">
                        <el-switch
                ...
                <el-table-column label="私有域名" :width="tableWidth">
                    <template slot-scope="scope">
                        <div
                            class=""
                            @click="admainClick(scope.row)"
                            v-if="scope.row.id"
                            style="
                                min-width: 200px;
                                min-height: 50px;
                        ...
                        <el-input
                            ref="domainInput"
                            v-if="domain_id == scope.row.id"
                            v-model="scope.row.domain"
                            @blur="editDomain(scope.row)"
                        ></el-input>
                    </template>
                </el-table-column>
            ...
            >
                <!-- 修改用户 -->
                <el-form :model="editForm" ref="ruleFormRef" label-width="66px">
...
export default {
    ...
    methods: {
        ...
        async updateUser() {
            if (
                this.editForm.nickname.length >= 2 ||
        ...
        async zengsongVip(e) {
            ...
            if (result === 'confirm') {
                if (this.isDone) return
                this.isDone = true
                let { data: res } = await this.$http.put('user/song7day', {
                    id: e.id
                })
                this.isDone = false
                if (res.code === 200) {
        ...
        async editDomain(row) {
            const { data: res } = await this.$http.put('user/updomain', {
                id: row.id,
                domain: row.domain
            })
            if (res.code === 200) {
